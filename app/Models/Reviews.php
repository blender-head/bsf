<?php 
    ...
    class Reviews extends BaseModel
    {
        ...
        {
            $query = "SELECT `r`.`id`, `r`.`uuid`, `r`.`title`, `r`.`slug`, `r`.`created_at`, `u`.`name` AS `author`, IF(`r`.`status` =  1, 'Published', 'Unpublished') AS `status`
                FROM `reviews` AS `r` INNER JOIN `users` AS `u` ON `r`.`created_by` = `u`.`id` 
        ...
        public function setStatus(array $data): bool 
        {
            ...
            DB::update("UPDATE `reviews` SET `status` = ? WHERE `uuid` IN (" . $in_placeholders . ")", $prepared_data);
        ...
        public function deleteData(array $data): bool 
        {
            ...
            DB::update("DELETE FROM `reviews` WHERE `uuid` IN (" . $in_placeholders . ")", $prepared_data);
        ...
        {
            $query = "SELECT `r`.`id`, `r`.`title`, `r`.`content`, `r`.`excerpt`, `r`.`filename`, `r`.`filepath`, `r`.`created_at`, `r`.`updated_at`, `u`.`name` 
                FROM `reviews` AS `r` INNER JOIN `users` AS `u` ON `r`.`created_by` = `u`.`id` 
            WHERE `r`.`slug` = :slug";
        ...
        public function getImagesByUUID(array $data): array
        {
            ...
            $result = DB::select("SELECT `filename`FROM `reviews` WHERE `uuid` IN (" . $in_placeholders . ")", $prepared_data);
            return $result;
        }
        ...
        public function prepareIndexData(array $db_data, string $row_number): array
        {
            ...
            $edit_url = BaseModuleRoute::BE_PREFIX . '/reviews/edit';

            foreach ($db_data as $result) 
            {
                $input = [
                    View::make('partials.selector', ['uuid' => $result->uuid, 'content_type' => 'reviews'])->render(),
                    $row_number,
                    $result->title,
                    $result->author,
